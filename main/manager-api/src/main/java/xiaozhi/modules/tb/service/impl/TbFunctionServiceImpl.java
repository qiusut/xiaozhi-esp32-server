...
@Service
public class TbFunctionServiceImpl extends ServiceImpl<TbFunctionDao, TbFunctionEntity> implements TbFunctionService {
    ...
    public Page<TbFunctionEntity> deviceTypeList(Integer curPage, Integer limit, String type, String name) {
        ...
        queryWrapper.lambda().orderByDesc(TbFunctionEntity::getCreateDate);
    ...
    public void initFunctionRedis(){
        ...
        if(CollectionUtil.isNotEmpty(list)) {
            //先清除缓存
            redisUtils.delete("tb:device");
            Set<String> keys = redisUtils.getRedisTemplate().keys("tb:device_fun:*");
    ...
    private void saveFunction(TbFunctionEntity tbFunction) {
        ...
        for(int i=0;i<tbFunction.getFuns().size();i++){
            TbFunctionEntity.Function_call function_call = tbFunction.getFuns().get(i);
